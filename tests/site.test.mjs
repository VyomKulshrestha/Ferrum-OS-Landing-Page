...
test('the cinematic has eight complete, unique, statically rendered chapters', async () => {
  ...
  assert.match(controller, /window\.addEventListener\('load', scheduleOpeningMotion/)
  assert.match(controller, /window\.setTimeout\(\(\) =>/)
  assert.match(controller, /ensureAdjacentSources\(activeIndex\)/)
  assert.doesNotMatch(controller, /ensureAdjacentSources\(0\)/)
  assert.match(controller, /localProgress >= 0\.35[\s\S]*loadSceneSource\(nextIndex, 'auto'\)/)
  assert.match(html, /rel="preload" as="font" type="font\/woff2" href="\/fonts\/manrope-latin\.woff2" crossorigin/)
